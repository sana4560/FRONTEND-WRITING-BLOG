  ...
  useEffect(() => {
    const postData = async () => {
      if (!isEmailValid(email)) {
        showWarningSnackbar(
          "Please enter a valid email (e.g., email@gmail.com)"
        );
        setSubmitted(false);
        return;
      }
      ...
      if (!isPasswordValid(password)) {
        showWarningSnackbar(
          "Password must be at least 8 characters long and contain 1 alphabet and 1 special character."
        );
        setSubmitted(false);
        return;
      }
    ...
    showSuccessSnackbar,
    showWarningSnackbar,
  ]); // Only dependencies related to submission
