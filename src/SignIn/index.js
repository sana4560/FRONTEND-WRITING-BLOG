...
export default function SignIn({ open, onClose, handleSignUpOpen }) {
  ...
  }, [submitted, email, password, showSuccessSnackbar,showWarningSnackbar, showUsername, showUserId]);
  ...
  useEffect(() => {
    if (forgetPassword && email) {
      const generateMail = async () => {
        try {
          const response = await fetch('http://localhost:8000/users/forget-password', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
  ...
  return (
    ...
    >
      <DialogTitle sx={{ margin: '0px', textAlign: 'center', height: '40px', background: 'primary.main' }}>Sign In</DialogTitle>
      ...
      <Typography variant="body2" sx={{ textAlign: 'center', margin: '10px' }}>
        Don't have an account? <Button onClick={handleSignUpOpen}>Sign Up</Button>
      </Typography>

    
      <Button onClick={handleForgetPassword} sx={{ display: 'block', paddingBottom: '25px' }}>
        Forget Password
      </Button>
